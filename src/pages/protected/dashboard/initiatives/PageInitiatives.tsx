...
import { communityEvents, contentEvents } from "@/lib/clarityEvents"

export default function PageInitiatives() {
  const [stateFilter, setStateFilter] = useState("proposal")
  const [search, setSearch] = useState("")

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError } = useInitiatives(stateFilter)
  ...
  useEffect(() => {
    if (stateFilter === InitiativeState.PROPOSAL) {
      contentEvents.viewInitiativesHome()
      communityEvents.filterUsed({ filterName: InitiativeState.PROPOSAL })
    } else if (stateFilter === InitiativeState.INPROCESS) {
      communityEvents.filterUsed({ filterName: InitiativeState.INPROCESS })
    } else if (stateFilter === InitiativeState.APPROVED) {
      contentEvents.initiativePublishedActive()
      communityEvents.filterUsed({ filterName: InitiativeState.APPROVED })
    }
  }, [stateFilter])
      ...
      <Box className="flex flex-col flex-1 p-6 gap-6 max-w-7xl">
        <Box className="flex gap-6 items-center justify-center">
          <Paper elevation={2} className="grid grid-cols-3 bg-white rounded-full">
            <IconButton
              aria-label={InitiativeState.PROPOSAL}
              onClick={() => stateFilter !== InitiativeState.PROPOSAL && setStateFilter(InitiativeState.PROPOSAL)}
            ...
            >
              <StarBorderRoundedIcon
                className={`transition-colors duration-500 ease-in-out text-[#FF7875] group-hover:bg-[#DCE2F9] px-3 py-1 rounded-2xl w-16 h-8 ${stateFilter === InitiativeState.PROPOSAL && "bg-[#DCE2F9]"}`}
              />
              <Typography
                className={`text-xs text-gray-500 group-hover:font-medium ${stateFilter === InitiativeState.PROPOSAL && "font-medium text-[#000!important]"}`}
              >
                Propuestas
              </Typography>
            </IconButton>
            <IconButton
              aria-label={InitiativeState.INPROCESS}
              onClick={() => stateFilter !== InitiativeState.INPROCESS && setStateFilter(InitiativeState.INPROCESS)}
            ...
            >
              <StarHalfRoundedIcon
                className={`transition-colors duration-500 ease-in-out text-[#FA8C16] group-hover:bg-[#DCE2F9] px-3 py-1 rounded-2xl w-16 h-8 ${stateFilter === InitiativeState.INPROCESS && "bg-[#DCE2F9]"}`}
              />
              <Typography
                className={`text-xs text-gray-500 group-hover:font-medium ${stateFilter === InitiativeState.INPROCESS && "font-medium text-[#000!important]"}`}
              >
                En proceso
              </Typography>
            </IconButton>
            <IconButton
              aria-label={InitiativeState.APPROVED}
              onClick={() => stateFilter !== InitiativeState.APPROVED && setStateFilter(InitiativeState.APPROVED)}
            ...
            >
              <StarRateRoundedIcon
                className={`transition-colors duration-500 ease-in-out text-[#46A616] group-hover:bg-[#DCE2F9] px-3 py-1 rounded-2xl w-16 h-8 ${stateFilter === InitiativeState.APPROVED && "bg-[#DCE2F9]"}`}
              />
              <Typography
                className={`text-xs text-gray-500 group-hover:font-medium ${stateFilter === InitiativeState.APPROVED && "font-medium text-[#000!important]"}`}
              >
                Aprobadas
              </Typography>
            </IconButton>
        ...
        <Box className="flex flex-wrap gap-6 justify-center">
          ...
          )?.map((initiative: InitiativeMin) => <InitiativeCard initiative={initiative} key={initiative.id} />)}
        </Box>
        ...
        <Box className="mt-12 rounded-2xl overflow-hidden bg-blue-500 text-white flex flex-col md:flex-row">
          ...
          <Box component="img" src={teamImg} alt="" className="w-full md:flex-1 min-w-0 object-cover" />
        </Box>
      </Box>
    </Box>
  )
