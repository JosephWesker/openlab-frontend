...
// import type { Announcement, Collaborator, Initiative } from "../../schemas/initiativeSchema"
...
import type { Collaborator, Initiative } from "../../../initiative/schemas/initiativeSchema"
import CollaborationApplyModal from "./components/collaboration-apply-modal"
import InvestApplyModal from "./components/invest-apply-modal"
// import type { Collaborator, Initiative } from "../../../initiative/schemas/initiativeSchema"
// import { useUpdateUserApplications } from "./hooks/useUpdateUserApplications"

export default function Sidebar({ initiative }: { initiative: Initiative }) {
  const localInitiative = initiative

  // const [localInitiative, updateInitiative] = useState<Initiative>(initiative)
            ...
            <Box className="flex flex-wrap gap-4">
              <Box className="flex flex-wrap gap-2">
                ...
                <Box className="flex flex-wrap gap-2 w-full">
                  {localInitiative.externalLinks.dework && <ChipWithTooltip appName={GovernanceLinks.DEWORK} url={localInitiative.externalLinks.dework}/>}
                  {localInitiative.externalLinks.discord && <ChipWithTooltip appName={GovernanceLinks.DISCORD} url={localInitiative.externalLinks.discord}/>}
                </Box>
              </Box>
              <Box className="flex flex-wrap gap-2">
                <Typography>Gobernanza</Typography>
                <Box className="flex flex-wrap gap-2 w-full">
                  {localInitiative.externalLinks.aragon && <ChipWithTooltip appName={GovernanceLinks.ARAGON} url={localInitiative.externalLinks.aragon}/>}
                  {localInitiative.externalLinks.dtoCreateToken && <ChipWithTooltip appName={GovernanceLinks.DTO_CREATE_TOKEN} url={localInitiative.externalLinks.dtoCreateToken}/>}
                </Box>
              </Box>
              <Box className="flex flex-wrap gap-2">
                <Typography>Repositorios</Typography>
                <Box className="flex flex-wrap gap-2 w-full">
                  {localInitiative.externalLinks.githubFront && <ChipWithTooltip appName={GovernanceLinks.GITHUB_FRONT} url={localInitiative.externalLinks.githubFront}/>}
                  {localInitiative.externalLinks.githubBack && <ChipWithTooltip appName={GovernanceLinks.GITHUB_BACK} url={localInitiative.externalLinks.githubBack}/>}
                </Box>
              </Box>
            </Box>
          :
            <Card className="flex flex-col items-center rounded-xl p-10 gap-4 bg-linear-to-t from-[#DEE8FF] to-[#FFFFFF]">
              ...
              <Typography className="text-xs text-[#304578] text-center">
                Una vez que esta propuesta sea aprobada, aquí encontrarás los enlaces directos a su
                DAO en aragon de esta iniciativa, a la comunidad en Discord, su repositorio en GitHub
                y su tablero de tareas en Dwork.
              </Typography>
            </Card>
          }
        </CardContent>
        ...
        <CardContent className="p-4 flex flex-col gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <Typography className="w-full max-w-[12rem] md:max-w-[18rem] lg:max-w-[18rem] xl:max-w-[18rem]"
          ...
          {announcements.length === 0 && cofounders.length === 0 &&
            <CardContent className="flex flex-col items-center p-4 gap-4">
              <Card className="flex flex-col items-center rounded-xl p-10 gap-4 bg-linear-to-t from-[#DEE8FF] to-[#FFFFFF]">
                ...
                <Typography className="text-xs text-[#304578] text-center">
                  Busca a tus socios estratégicos. Publica las vacantes de cofundador para construir el núcleo de tu equipo.
                </Typography>
              </Card>
            </CardContent>
          }
          ...
          {/* {announcements.map((announcement: Announcement, index: number) => (
            <div key={index} className="flex items-center space-x-2">
              ...
              <div><p className="text-sm">{announcement.gSkill}</p></div>
              {
          ...
          {hasRole(UserRole.COLLABORATOR) ? (
            ...
            <CardContent className="flex flex-col items-center p-4 gap-4">
              <Card className="flex flex-col items-center rounded-xl p-10 gap-4 bg-linear-to-t from-[#DEE8FF] to-[#FFFFFF]">
                ...
                <Typography className="text-xs text-[#304578] text-center">
                  Tu proyecto está listo para crecer. Invita a colaboradores para que aporten su talento a tu iniciativa.
                </Typography>
              </Card>
            </CardContent>
          )}
          ...
          {hasRole(UserRole.INVESTOR) ? (
            ...
            <CardContent className="flex flex-col items-center p-4 gap-4">
              <Card className="flex flex-col items-center rounded-xl p-10 gap-4 bg-linear-to-t from-[#DEE8FF] to-[#FFFFFF]">
                ...
                <Typography className="text-xs text-[#304578] text-center">
                  Esta iniciativa está lista para ser impulsada. Sé uno de los primeros en creer en su potencial y
                  ayuda a catalizar su crecimiento.
                </Typography>
              </Card>
            </CardContent>
          )}
